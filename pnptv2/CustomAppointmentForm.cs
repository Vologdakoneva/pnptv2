...
namespace pnptv
{
    public partial class CustomAppointmentForm : XtraForm, IDXManagerPopupMenu
    {
        ...
        public CustomAppointmentForm(SchedulerControl control, Appointment apt, string Connectionstrin)
            : this(control, apt, false, Connectionstrin)
        {
        }
        public CustomAppointmentForm(SchedulerControl control, Appointment apt, bool openRecurrenceForm, string Connectionstring)
        {
        ...
        public virtual void LoadFormData(Appointment appointment)
        {
            ...
            if (appointment.Id == null)
                   appointment.SetId( -1 );
            if (appointment.CustomFields["IDALL"] == null)
                appointment.CustomFields["IDALL"] = -1;
        }
        ...
        protected virtual void BindControllerToControls()
        {
            ...
            //BindProperties(this.tbSubject, "EditValue", "StatusId", DataSourceUpdateMode.OnPropertyChanged);
            BindProperties(this.tbSubject, "EditValue", "Location", DataSourceUpdateMode.OnPropertyChanged);
            //BindProperties(this.tbSubject, "Text", "Subject");
            BindProperties(this.tbDescription, "Text", "Description", DataSourceUpdateMode.OnPropertyChanged);
        ...
        private void edtStartTime_Move(object sender, EventArgs e)
        {
        ...
        {
            // TODO: This line of code loads data into the 'qDoctor.DOCTORS' table. You can move, or remove it, as needed.
            ...
            {
                tbSubject.EditValue = qDoctor.DOCTORS.Rows[0]["IDALL"];
                Controller.StatusId = Convert.ToInt32( qDoctor.DOCTORS.Rows[0]["IDALL"]);
                tbDescription.Text = qDoctor.DOCTORS.Rows[0]["FIO"].ToString();
            }
        ...
        {
            if (qDoctor.DOCTORS.Rows.Count > 0) {
                Controller.StatusId = Convert.ToInt32(tbSubject.EditValue);
                Controller.Location = tbSubject.EditValue.ToString();
                tbDescription.Text = tbSubject.Text;
                //this.tbSubject.DataBindings["EditValue"].WriteValue();
                //_apt.CustomFields["IDVRACH"] = Convert.ToInt32(tbSubject.EditValue);
                ////storage.
            }
        }
    }
